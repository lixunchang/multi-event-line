...
export const analysisLineData=(lines:any, fieldNames: any = {})=>{
  ...
  return lines.reduce(
    ({ minDt, maxDt,minValue, maxValue }: any, event: any) => {
      return {
...
export const analysisEventLineData=(events: any, lines: any, config: Record<string,any> = {})=>{
  const { forward = 30, behind = 30, unit = 'day', fieldNames={}, scale } = config||{};
  const { eventStart = 'startDate', eventEnd = 'endDate', lineXField = 'dt', lineYField = 'value', lineSeriesField='type' } = fieldNames||{};

  const { min: eventMinDate, max: eventMaxDate } = analysisEventData(events,{eventStart,eventEnd});
  const { minDt: lineMinDate, maxDt: lineMaxDate, minValue: lineMinValue, maxValue: lineMaxValue} = analysisLineData(lines,{lineXField,lineYField,lineSeriesField});
